...
from lmfit import minimize, Parameters, conf_interval, printfuncs
...
def SetupParametersDictionary( i, Params,  Temperature ):
    ...
    Params.add('TCutW_%i'       %(i+1), value=TCutW,
                                        ...
                                        vary=False)
    Params.add('Temperature_%i' %(i+1), value=Temperature,
...
def FitData( DataSets, Params, AveragingType, ThetaAngles, ProbCurveType, LabelData):
    ...
    Result = minimize( Residual, Params, args=(X, Y, DataSets, AveragingType, ThetaAngles, ProbCurveType) )
...
def TOF(Time, NDataSet, Params, AveragingType, ThetaAngles, ProbCurveType, Debug=False):
    ...
    MaxTOF   = Params['MaxTOF_%i'    %NDataSet].value
...
def Debug1(Time=2E-5):
    ...
    FFRDist     = Params['FFRDist_1'    ].value
    MaxTOF      = Params['MaxTOF_1'     ].value
...
def Prob(En, NDataSet, Params, ProbCurveType):
    ...
    B  = Params['B_%i' %NDataSet].value
    BI = Params['BI_%i' %NDataSet].value
    C  = Params['C_%i' %NDataSet].value
...
def ProbFromTOFInversion(Time, Signal, NDataSet, Params, AveragingType, ThetaAngles, ProbCurveType):
    ...
    if AveragingType == "PointDetector":    
        ...
        for Theta in ThetaAngles:
            ...
            Ekin = (0.5 * MassAmu * Velocity**2.) * eVConst
            Enorm = Ekin * np.cos( np.radians(Theta) )**2. # Reaction probability depends on normal energy
            VelocityDistribution = VelocityDistribution + Velocity**4. * np.exp( -Ekin / (kb * Temperature) ) * np.cos( np.radians(Theta) )**2. * np.sin( np.radians(Theta) ) * ThetaStep
...
def WriteTOFOutput( TOFTime, TOFData, State, Label, NDataSet, Params, AveragingType, ThetaAngles, ProbCurveType):
    ...
    MinTime =  Params['TimeCorr_1'].value*1.E-6 + 0.1E-6
    ...
    print("TOF data fitted written to file ", FileNoBackground)       
    ...
    print("Fitted TOF spectrum written to file ", TOFOutFile)
    return
...
def WriteProbOutput( TOFTime, TOFData, State, Label, NDataSet, Params, AveragingType, ThetaAngles, ProbCurveType):
    ...
    print("Reaction probability curve written to file ", ReactionProbabilityOutFile)
    ...
    if AveragingType != "LineDetector":
        ...
        np.savetxt( TOFInvertedOutFile, np.column_stack(( TOFEnergy, TOFReactionProbability*Params['MaxTOF_%i' %NDataSet].value, Prob( TOFEnergy, NDataSet, Params, ProbCurveType)*Params['MaxTOF_%i' %NDataSet].value )))
        print("Inverted TOF written to file ", TOFInvertedOutFile)
    return                                               
...
# Check the arguments given while launching the program                                                       
parser = argparse.ArgumentParser(description='TOF fitting program')
                                                       

parser.add_argument('-t', '--task', help = 'string; Select the task to be performed: "FitTOF", or "Calibration"', required = True, choices = ['FitTOF', 'Calibration'])
parser.add_argument('-l', '--label', help = 'string; Arbitrary label added to the output files', required = True)                                                      
#parser.add_argument('-r', '--rovib', help = 'string; Rovibrational State (e.g. "v0j0")')
parser.add_argument('-f', '--function', help = 'string; Type of Prob. function used in the model: "ERF", "GMP", "LGS", "FPC", "TPC"', choices = ['ERF', 'GMP', 'LGS', 'SPL', 'FPC', 'TPC'], default = 'ERF' )
parser.add_argument('-i', '--input', help = 'string; Input file containing TOF spectrum. For multiple datasets, insert file names separated by commas.', required = True )
parser.add_argument('-a', '--angularaveraging', help = 'string; Angular averaging type: "None", "PointDetector", "LineDetector"', choices = ["None", "PointDetector", "LineDetector"],  required = True)
parser.add_argument('-b', '--background', help = 'string; Background file. For multiple datasets, insert file names separated by commas.', default = "" )
parser.add_argument('--baseline', help = ' fit baseline with form y = k ', action='store_true' )                                                       
parser.add_argument('--cutoff', help = ' fit cutoff function parameters', action='store_true' )                                                       
parser.add_argument('--corrtime', help = ' fit correction time', action='store_true')
parser.add_argument('--temperature', help = ' fit surface temperature', action='store_true' )
parser.add_argument('--mintime', help= 'decimal; fit TOF starting from time (in us)', default = "" )
parser.add_argument('--maxtime', help= 'decimal; fit TOF until to time (in us)', default = "" )
...
if DataType == 'Calibration':
    ...
    ProbCurveType = args.function
DataFiles = args.input.split(",")
AveragingType = args.angularaveraging
BackgroundFiles = args.background.split(",")
...
FitBaseline = args.baseline
FitCutoffFunction = args.cutoff
FitCorrectionTime = args.corrtime
FitTemperature = args.temperature
Tmin = args.mintime
Tmax = args.maxtime
...
SetFreeParameters( Params, DataType, ProbCurveType, FitTemperature, FitCutoffFunction, FitCorrectionTime, FitBaseline )
...
# Fit TOF
FitData( DataSets, Params, AveragingType, ThetaAngles, ProbCurveType, Label )
...
for i in range( len( DataSets )):
    ...
    WriteTOFOutput( Time, Signal, State, Label, NDataSet, Params, AveragingType, ThetaAngles, ProbCurveType)

    if DataType != 'Calibration':
        WriteProbOutput( Time, Signal, State, Label, NDataSet, Params, AveragingType, ThetaAngles, ProbCurveType)                                                   
